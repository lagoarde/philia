...
'*** GREEN LENDING RISK 
' this variable is comprised between 0 and 1 and impacts banks' perception of green credit demand. It is set to 1 in the baseline scenario
'*** It is featured in equation (32.1) and (32.2) (defining banks lending risk for green coop and green listed firms' loans) 
series shock_risk_k
shock_risk_k=1
...
'Alternative disposable incpme and consumption (robustness check)
series C_w_hs
C_w_hs=C_w
series  C_r_w_hs
C_r_w_hs=C_r_w
...
'PRODUCTIVE CAPITAL ASSETS 'numbers have changed here
 
philia.append K=K_k+K_p+K_c 'eq 1.4
philia.append DA=DA_k+DA_c+DA_p 'eq 1.5
philia.append K_b=K_k_b+K_c_b+K_p_b 'eq 1.6
philia.append K_g=K-K_b 'eq 1.7
...
'ALTERNATIVE INFLATION AND WAGE BILL 'this is new

'NEW WAGE BILL

'philia.append iota_wb=(omega_w*ws_w+phi_f*ws_f)/(omega_w+phi_f)  'eq 2.5
philia.append iota_wb=(omega_w*iota_t_w+phi_f*iota_t_f)/(omega_w+phi_f)  'eq 2.5
philia.append pi_f=((omega_w*phi_f)*(iota_t_w-iota_t_f))/(omega_w+phi_f) 'eq 2.6 
philia.append pi=pi_f + epsi_fi 'eq 2.7
philia.append epsi_fi=epsi_fi(-1)+pi_clim 'eq.2.8
philia.append pi_clim=ecosystemic_shock*@recode(@date<@dateval("152"), 0, omicron*((depl_e(-1)+depl_m(-1)) *ecosystemic_shock*d_t(-1)))   'eq 2.9
philia.append omicron=0.1 '  eq 2.9 

philia.append iota_t_w=iota_t_w(-1)/(1*(1+pi)) 'eq 2.10
'new
philia.append omega_w=omega_w(-1)/(1*(1+pi)) 'eq 2.11
 'eq 2.11 & e 2.12 are parameters
philia.append price=price(-1)*(1+pi) 'eq 2.12
philia.append real_Y =Y_r(-1)+ (Y-Y(-1))/(1+pi) 'eq.2.13 EN GARDER UNE SEULE POUR L'INSTANT C'EST YR QUI EST UTILISE DANS ECOSYSTEME
philia.append Y_r =Y/price ' eq.2.13 alt
philia.append WB=iota_WB(-1)*Y '  'eq 2.14
philia.append WB_c=0.10*WB '   'eq 2.14
philia.append WB_k=0.75*WB '  'eq 2.14

'INFLATION AND REAL GDP: A ENLEVER

'was ok

'philia.append pi=@recode(@date<@dateval("152"), 0, (pi_a+ pi_clim + pi_f)) 'eq.1.4
'philia.append pi_a= pi(-1)+ 0.05*(pi(-1)-pi_a(-1)) 'eq 1.5 was OK
'philia.append pi_f =shock_inf*0.000001*((WB(-1)-WB(-2))/Y(-2)) 'eq 1.6 BEFORE REFEREE
'philia.append pi_f =shock_inf*0.000001*d(WB(-1)/Y(-1))  'eq 1.6 AFTER REFEREE
...
philia.append T_r=teta_r*(div_s(-1)+i_d*(M_d_r(-1))) 'eq. 3.2
...
'philia.append gamma20= @recode(gamma20_n<0.8, gamma20_n,0.8) 'eq 8.1
'philia.append gamma10=gamma10(-1)-(gamma20-gamma20(-1)) 'eq 8.1 
'philia.append gamma20_n=gamma20_n(-1)*(1+omega*ecosystemic_shock*d_t(-1))
'philia.append gamma20_n=gamma20_n(-1)*(1-omega*ecosystemic_shock*d_t(-1))
...
philia.append f_d_k_g  =@recode(inv_d_k=0,0, @recode(inv_d_k_g   - PU_k * ((inv_d_k_g  / inv_d_k))>0  , inv_d_k_g   - PU_k  * ((inv_d_k_g  / inv_d_k)), 0)) 'eq 26.2  
philia.append f_d_k_b=@recode(inv_d_k=0,0, @recode(inv_d_k_b- PU_k*((inv_d_k_b/inv_d_k))>0,inv_d_k_b- PU_k*((inv_d_k_b/inv_d_k)),0)) 'eq 26.3  
philia.append e_s_k=@recode(f_d_k-f_s_k>0, f_d_k-f_s_k, 0) 'eq 26.4  

'Investment spending

philia.append inv_s_k=inv_s_k_g+inv_s_k_b 'eq 27.1
philia.append inv_s_k_g=@recode(inv_d_k>0, f_s_k_g + PU_k* (inv_d_k_g/inv_d_k) + (SMC_share_k)*smc + e_d_k*(inv_d_k_g/inv_d_k),0 ) 'eq 27.2
philia.append inv_s_k_b=@recode(inv_d_k>0, f_s_k_b + PU_k* (inv_d_k_b/inv_d_k) - (SMC_share_k)*smc+ e_d_k*(inv_d_k_b/inv_d_k), 0) 'eq 27.3
...
philia.append LR_c_g=shock_risk_c*@recode(lr_a1*(stock_L_s_c/K_c) - lr_b1*(d(y)/y(-1)) + lr_c1*i_e>0,lr_a1*(stock_L_s_c/K_c) - lr_b1*(d(y)/y(-1)) + lr_c1*i_e,0) 'eq 32.1 CHANGEMENT VARIABLE SHOCK_RISK_K ICI
philia.append LR_k_g=shock_risk_k*@recode(lr_a1*(stock_D_s_K/(K_k))  - lr_b1*(d(y)/y(-1)) + lr_c1*i_e<0.7, lr_a1*(stock_D_s_K/(K_k))  - lr_b1*(d(y)/y(-1)) + lr_c1*i_e, 0.7) 'eq 32.2 CHANGEMENT VARIABLE SHOCK_RISK_C ICI
'philia.append LR_c_b=LR_c_g*(1+psi*ecosystemic_shock*d_t(-1)) 'eq 32.3 DELETE
'philia.append LR_k_b=LR_k_g*(1+psi*ecosystemic_shock*d_t(-1)) 'eq 32.4  DELTE
philia.append LR_c_b=@recode(lr_a1*(stock_L_s_c/K_c) - lr_b1*(d(y)/y(-1)) + lr_c1*i_e>0,lr_a1*(stock_L_s_c/K_c) - lr_b1*(d(y)/y(-1)) + lr_c1*i_e,0)*(1+psi*ecosystemic_shock*d_t(-1)) 'eq 32.3  NEUTRALISE SHOCK_RISK_C
philia.append LR_k_b=@recode(lr_a1*(stock_D_s_K/(K_k))  - lr_b1*(d(y)/y(-1)) + lr_c1*i_e<0.7, lr_a1*(stock_D_s_K/(K_k))  - lr_b1*(d(y)/y(-1)) + lr_c1*i_e, 0.7)*(1+psi*ecosystemic_shock*d_t(-1)) 'eq 32.4  NEUTRALISE SHOCK_RISK_C
...
philia.append p_b_flow  = i_gb * d(stock_GB_d_b(-1))  + i_l_k_b  * l_d_k_b(-1)  + i_l_k_g  * l_d_k_g(-1)  + i_l_c_b  * l_d_c_b(-1)  + i_l_c_g  * l_d_c_g(-1)  + i_cp_b  * CP_d_k_b(-1)  + i_cp_b  * CP_d_k_g(-1)  + i_b_g  * B_d_k_g(-1)  + i_b_b  * B_d_k_b(-1)  + i_mr  *(d(stock_H_d_m(-1)))  + i_df  * d(stock_H_ex(-1))  - i_d*d(stock_m_s_h(-1)) - i_e  * (d(stock_A_s_b(-1)))   'eq 37.1
philia.append div_b  = @recode(p_b_flow>0  , p_b_flow  , 0) 'eq 37.2


'Banks' balance sheet

philia.append BA_a  = p_gb  * stock_GB_d_b + p_l_k_b  * stock_l_s_k_b  + p_l_k_g  * stock_l_s_k_g  + p_l_c_b  * stock_l_s_c_b  + p_l_c_g  * stock_l_s_c_g  +p_b_b  * stock_b_s_k_b  +p_cp_k_b  * stock_cp_s_k_b  + p_cp_k_g  * stock_cp_s_k_g + p_b_g  * stock_b_s_k_g+ (stock_h_d_m-stock_h_d_h)    ' eq 38.1
philia.append BA_e  = p_gb  * stock_GB_d_b + p_l_k_b  * stock_l_d_k_b  + p_l_k_g  * stock_l_d_k_g  + p_l_c_b  * stock_l_d_c_b  + p_l_c_g  * stock_l_d_c_g  +p_b_b  * stock_b_d_k_b  +p_cp_k_b  * stock_cp_d_k_b  + p_cp_k_g  * stock_cp_d_k_g + p_b_g  * stock_b_d_k_g+ (stock_h_d-stock_h_d_h)  'eq 38.2 
...
philia.append BE_s=-((d(f_s_k+l_s_c))+gb_d_b+(h_d-d(qe_t_s)-d(stock_h_d_h))-d(stock_a_s_b)-d(stock_m_s_h))
 

'Auxiliary
philia.append BRA_a = stock_l_s_k_b  +  stock_l_s_k_g  + stock_l_s_c_b  +stock_l_s_c_g  + stock_b_s_k_b  +stock_cp_s_k_b  +  stock_cp_s_k_g + stock_b_s_k_g   'Stocks of risky assets pre QE
philia.append BRA_e = stock_l_d_k_b  + stock_l_d_k_g  +  stock_l_d_c_b  + stock_l_d_c_g  +stock_b_d_k_b  +stock_cp_d_k_b  +  stock_cp_d_k_g +  stock_b_d_k_g  'Stocks of risky assets post QE

'Capital adequacy ratio (CAR)
 
philia.append car_a=be_a/wr_a 'eq 38.6
philia.append car_e=be_e(-1)/wr_e 'eq 38.7
philia.append wr_max  = be_e(-1) / car_t 'eq 38.8 
philia.append wr_a = n_1  * (p_l_k_b  * stock_l_d_k_b  + p_l_k_g  * stock_l_d_k_g  + p_l_c_b  * stock_l_d_c_b  + p_l_c_g  * stock_l_d_c_g + p_b_b  * stock_b_d_k_b  + p_b_g  * stock_b_d_k_g + p_cp_k_b  * stock_cp_d_k_b  + p_cp_k_g  * stock_cp_d_k_g) 'eq 38.9
philia.append qe_t= shock_qe*@recode(wr_a(-1)<wr_max(-1), 0, (wr_a(-1) - wr_max(-1))/n_1) 'eq 38.10 
philia.append qe_t_s=qe_t_s(-1)+qe_t 'eq 38.11
philia.append wr_e  = wr_a-qe_t*n_1 'eq 38.12
...
philia.append tilda  = @recode((stock_gb_d_b(-1)  + stock_h_d(-1)-stock_h_d_h(-1))/(stock_M_s_h(-1))*1.008>0,(stock_gb_d_b(-1)  + stock_h_d(-1)-stock_h_d_h(-1))/(stock_M_s_h(-1))*1.008, (stock_gb_d_b(-1)  + stock_h_d(-1))/(stock_M_s_h(-1))*(1+gamma10)) '38.14
philia.append stock_GB_d_b_t  = @recode(tilda  * stock_M_s_h  * lcr_t  - stock_h_d>0  , tilda  * stock_M_s_h  * lcr_t  - stock_h_d  , 0) 'eq 38.15
philia.append gb_d_b_t  = stock_GB_d_b_t  - stock_GB_d_b(-1) 'eq 38.16
philia.append lcr_e = (stock_gb_d_b+ (stock_h_d-stock_h_d_h)) / (tilda * stock_M_s_h) 'eq 38.17

'Auxiliary
philia.append stock_gb_d_b  = @recode (stock_gb_d_b(-1)  + gb_d_b_t(-1)<stock_gb_s(-1), stock_gb_d_b(-1)  + gb_d_b_t(-1)  , stock_gb_s(-1)) ' 
philia.append gb_d_b  = d(stock_gb_d_b) ' 
...
philia.append e_d_k=@recode((S_s)>e_s_k, e_s_k, (S_s)) '39.2 old
'philia.append e_d_k=@recode(S_s-be_s>0, @recode((S_s)-be_s>e_s_k, e_s_k, (S_s-be_s)), 0) 'new

philia.append stock_gb_d_s=stock_gb_d_s(-1)+gb_d_s 'eq 39.3
'philia.append gb_d_s=@recode(gb_s-gb_d_b>0, gb_s-gb_d_b, 0) 'eq 39.4 old

philia.append gb_d_s=@recode(gb_s-gb_d_b>0, @recode(gb_s-gb_d_b<S_s-e_d_k-be_s, gb_s-gb_d_b, S_s-e_d_k-be_s), 0) '39.4 NEW
 '
'philia.append s_xs=(s_s-e_d_k-(be_s))-gb_d_s 'eq 39.5  old
philia.append s_xs=@recode((s_s-e_d_k-(be_s))-gb_d_s>0,(s_s-e_d_k-(be_s))-gb_d_s ,0)  'eq 39.5
...
philia.append qe_gb=@recode(gb_s-gb_d_b-gb_d_s>0,gb_s-gb_d_b-gb_d_s ,0) 'eq 40.14 NEW
...
philia.append cb_income = i_l_k_b*stock_qe_l_k_b(-1) + i_l_k_g*stock_qe_l_k_g(-1) + i_l_c_b*stock_qe_l_c_b(-1)+ i_l_c_g*stock_qe_l_c_g(-1) + i_b_b(-1)*stock_qe_b_b(-1)+i_b_g(-1)*stock_qe_b_g(-1)+i_cp_b(-1)*stock_qe_cp_b(-1)+i_cp_g(-1)*stock_qe_cp_g(-1)+ i_e*stock_a_s_b(-1) 'eq 42
philia.append cb_cost = i_mr*stock_h_d_m(-1)+i_df*(stock_h_ex(-1)-stock_h_smc(-1)) 'eq 42
philia.append f_cb= cb_income - cb_cost 'eq 42
...
philia.append i_mr=i_df-0.0001 'eq.45
...
'philia.append i_l_c_g  = @recode(lr_c_g>lr_k_g  , i_l_k_g  + (lr_c_g  - lr_k_g)  - sig_i_5  * (1  + gamma_i  * ecosystemic_shock*d_t(-1))    , i_l_c_g(-1) - sig_i_5  * (1  - gamma_i  * ecosystemic_shock*d_t(-1))  ) 'eq 46.6
...
'philia.append i_cp_b  =  i_e  + lr_k_b  + sig_i_1 +  sig_i_5  * (greenium)   'eq 46.2
'philia.append i_cp_g  = i_e  + lr_k_g  + sig_i_1 -  sig_i_5  * (greenium)  'eq 46.3
'philia.append i_l_k_g  = i_cp_g  + sig_i_2 - sig_i_5  * (greenium)  'eq 46.4
'philia.append i_l_k_b  = i_cp_b  + sig_i_2 +  sig_i_5  * (greenium)  'eq 46.5
'philia.append i_l_c_g  = i_l_k_g  + (lr_c_g  - lr_k_g)  - sig_i_5  * (greenium)    'eq 46.6
'philia.append i_l_c_b  =   i_l_k_b  + (lr_c_b  - lr_k_b)+ sig_i_5  * (greenium)  'eq 46.7
'philia.append i_b_b  =  i_l_k_b  +  sig_i_5  * (greenium)   'eq 46.8
'philia.append i_b_g  =  i_l_k_g - sig_i_5  * (greenium)   '46.9


'NON ADDITIVE GREENIUM
philia.append i_cp_b  =  i_e  + lr_k_b  + sig_i_1 +  sig_i_5  * (greenium)   'eq 46.1
philia.append i_cp_g  = i_e  + lr_k_g  + sig_i_1 -  sig_i_5  * (greenium)  'eq 46.2
philia.append i_l_k_g  = i_cp_g  + sig_i_2   'eq 46.3
philia.append i_l_k_b  = i_cp_b  + sig_i_2    'eq 46.4
philia.append i_l_c_g  = i_l_k_g  + (lr_c_g  - lr_k_g)       'eq 46.5
philia.append i_l_c_b  =   i_l_k_b  + (lr_c_b  - lr_k_b)   'eq 46.6
philia.append i_b_b  =  i_l_k_b     'eq 46.7
philia.append i_b_g  =  i_l_k_g  '46.8
...
'philia.append r_e_i_f=@recode(e_s_k>0, 0.01*(div_k/e_s_k), 0)
...
philia.append r_e_i_f=@recode(e_s_k>0, 0.01*(div_s/s_s(-1)), 0) 'eq 50.8 'NEW HERE
...
'philia.append gb_s=@recode(def_g-inv_s_p+da_p+p_p<0, -(def_g-inv_s_p+da_p+p_p), 0)' 53.1 'ICI J'AI RENUMEROTE ET TRANSFERE k_cb_s ET k_cb_s dans 53.1  
'philia.append def_g= t+k_cb_s+f_cb -(i_gb*gb_s(-1)+g) ' auxiliary

philia.append gb_s=@recode(g+i_gb*gb_s(-1)-(t+p_p+k_cb_s+f_cb)>0, (g+i_gb*gb_s(-1)-(t+p_p+k_cb_s+f_cb)), 0)' 53.1 'ICI J'AI RENUMEROTE ET TRANSFERE k_cb_s ET k_cb_s dans 53.1  
philia.append stock_gb_s  = stock_gb_s(-1)  + gb_s  'eq 53.2 
philia.append g=g_start+g_p 'eq 53.3
philia.append g_c  = 0.2*g 'eq 53.4
philia.append g_k  = g-g_c 'eq 53.5
philia.append g_p  = inv_s_p-da_p 'eq 53.6
...
'Green technology and energy consumption 'NUMBERS HAVE CHANGED + NEW EQUATIONS  + NEW VARIABLES: ACTIV_TEK, SHOCK_TEK_P, SHOCK_TEK

philia.append shock_tek_p=activ_tek*@recode(@date>@dateval("152"), d(k_g/k_b), 0) 'eq. 57.1
philia.append shock_tek=shock_tek(-1)+150*@movav(shock_tek_p, 20) 'eq.57.1

philia.append e =@recode(@date<@dateval("152"),e(-1),  1/(1+shock_tek)*epsilon*y_r/(0.0000035)) 'eq 57.2  
philia.append er = @recode(@date<@dateval("152"), er(-1), eta*e) 'eq 57.3
philia.append en = @recode(@date<@dateval("152"), en(-1), e-er) 'eq 57.4
philia.append ed =@recode(@date<@dateval("152"), ed(-1), er + en) 'eq 57.5
philia.append k_e = @recode(@date<@dateval("152"), k_e(-1), k_e(-1) + conv_e - en) 'eq 57.6
philia.append conv_e = @recode(@date<@dateval("152"), conv_e(-1), sigma_e*res_e(-1)) 'eq 57.7
...
philia.append C_w_hs=C_r_w_hs*price
philia.append C_r_w_hs=alfa_zero+alfa_one_w*(yd_hs_r_w) + alfa_two_w*(V_r_w(-1))
philia.append C_r_hs=C_r_r_hs*price
philia.append C_r_r_hs=alfa_zero+alfa_one_w*(yd_hs_r_r)
philia.append YD_hs_w=@recode(C_w +V_w(-1)-V_w(-2)>0, C_w +V_w(-1)-V_w(-2),0)  
philia.append YD_hs_r=@recode(C_r +V_r(-1)-V_r(-2)>0, C_r +V_r(-1)-V_r(-2),0)  
philia.append YD_hs_r_w=@recode(C_r_w +V_w(-1)/price(-1)-V_w(-2)/price(-2)>0,C_r_w +V_w(-1)/price(-1)-V_w(-2)/price(-2), 0) 
philia.append YD_hs_r_r=@recode(C_r_r +V_r(-1)/price(-1)-V_r(-2)/price(-2) >0, C_r_r +V_r(-1)/price(-1)-V_r(-2)/price(-2), 0)  
...
philia.append p_cp_k_b=@recode(p_cp_k_b(-1)*(1+shock_cb)>0,p_cp_k_b(-1)*(1+shock_cb), 0)
philia.append p_cp_k_g= 1
philia.append p_l_k_b=@recode(p_l_k_b(-1)*(1+shock_cb)>0,p_l_k_b(-1)*(1+shock_cb), 0)
philia.append p_l_k_g=1
philia.append p_l_c_b=@recode(p_l_c_b(-1)*(1+shock_cb)>0,p_l_c_b(-1)*(1+shock_cb), 0)
philia.append p_l_c_g=1
philia.append p_b_b=@recode(p_b_b(-1)*(1+shock_cb)>0,p_b_b(-1)*(1+shock_cb), 0)
...
'this scenario activates ecosystemic shocks without technological transition it is the baseline
